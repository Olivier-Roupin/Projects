...
static bool MainLoop(bool retryCreate)
{
	...
	//PROJET
	std::ofstream myfile;
	myfile.open("timestamps.txt", std::ofstream::out | std::ofstream::app);
	myfile << " NOUVEAU SUJET" << std::endl;

	//PROJET
	Flag * drapeau = nullptr;
	...
	static bool appui(false);
	static int mode(-1); //-1 non défini, 0 facile, 1 difficile

	//PROJET
	clock_t flagFrame = clock();
    ...
    while (Platform.HandleMessages())
    {
        ...
        if (sessionStatus.IsVisible)
        {
			...
			static float posy_clock = 0;
			static float Zaw_clock = 0;

			static Vector3f PosA(10.0f, 1.6f, 10.0f);
			static Vector3f Pos2(10.0f, 1.6f, 10.0f);
			static Vector3f Pos3(10.0f, 1.6f, 10.0f);
			
			if (mode != -1)
			{
				if (Platform.Key['Z'] || Platform.Key[VK_UP])     Pos3 += Matrix4f::RotationY(Yaw).Transform(Vector3f(0, 0, -0.05f));
				if (Platform.Key['S'] || Platform.Key[VK_DOWN])   Pos3 += Matrix4f::RotationY(Yaw).Transform(Vector3f(0, 0, +0.05f));
				if (Platform.Key['D'])							  Pos3 += Matrix4f::RotationY(Yaw).Transform(Vector3f(+0.05f, 0, 0));
				if (Platform.Key['Q'])                            Pos3 += Matrix4f::RotationY(Yaw).Transform(Vector3f(-0.05f, 0, 0));
				Pos2 = roomScene->Impact(PosA, Pos2, Pos3);
				Pos3 = Pos2;
				Pos2.y = roomScene->Hauteur(Pos2);
				...
				if (sessionStatus.HasInputFocus) // Pause the application if we are not supposed to have input.
					...
					//PROJET
				if (drapeau->Touch(Pos2))
				{
					drapeau->Next();
					flagFrame = clock();
					myfile << drapeau->Id << " " << clockToMilliseconds(flagFrame) << std::endl;
					if (mode)
						//chrono->SetTime(drapeau->Time(), clockInMilliseconds());
						chrono->GiveTime(drapeau->Time());
					else
						chrono->SetTime(0, clockInMilliseconds());
				}

				if (Platform.Key[' '] && !appui)
				{
					appui = true;
					flagFrame = clock();
					myfile << drapeau->Id << " " << clockToMilliseconds(flagFrame) << std::endl;
				...
				if (!Platform.Key[' '] && !Platform.Key['V'] && !Platform.Key['C'] && !Platform.Key['P'] && !Platform.Key['M'] && !Platform.Key['L'] && !Platform.Key['O'] && !Platform.Key['W'] && !Platform.Key['X'] && appui)
				{
					appui = false;
				}
			}
            ...
            for (int eye = 0; eye < 2; ++eye)
            {
				...
				//PROJET
				Matrix4f view0 = Matrix4f::LookAtRH(Vector3f(0, 0, 0), Vector3f(0, 0, 1), Vector3f(0, -1, 0));
				Matrix4f proj0 = ovrMatrix4f_Projection(hmdDesc.DefaultEyeFov[eye], 0.0001f, 1.01f, ovrProjection_FarLessThanNear);
				...
				if (mode != -1)
				{
					...
					//PROJET
					drapeau->Render(view, proj);
					chrono->PrintTime(clockInMilliseconds());
					chrono->Render(view0, proj0);
				}
