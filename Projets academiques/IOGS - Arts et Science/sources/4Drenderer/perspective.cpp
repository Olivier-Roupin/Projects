...
Donnees* perspective(Donnees* entree, int k, double* B, double* oeil, int r) //B : base orthonormale de l'espace d'arrivée (tableau d*k)
{
    int d(entree->d);
    int n(entree->n);
    Donnees* sortie;
    ...
    for(i = 0; i < n; i++)
    {
        ...
        for(j = 0; j < k; j++)
        {
            gsl_vector_set(vdif,j,gsl_vector_get(&vs.vector,j)-oeil[j]);
        }
        for(j = k; j < d; j++)
        {
            gsl_vector_set(vdif,j,-oeil[j]);
        }
...
Donnees* perspective(Donnees* entree, int k, double* B, int r) //B : base orthonormale de l'espace d'arrivée (tableau d*k)
{
    ...
    Ts = gsl_matrix_view_array(sortie->T, n, k);
    Tb = gsl_matrix_view_array(B, k, d);
    //Tb = gsl_matrix_view_array(base, k, d);
