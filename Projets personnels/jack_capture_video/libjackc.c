...
static int custom_samplerate=192000.000000;//256000.000000; //OLIVIER
...
static char *jackname="jack_capture_video"; // limited to 33 characters
...
static float *vu_vals_vec=NULL;
static float *frame_buffer=NULL;
static int frame_buffer_pos=0;
static int   pad_buffer_pos=0;
static float min_buffer=0;
static float max_buffer=0;
static int vu_vals_len=0;
static int vu_vals_new=0;
...
static void portnames_add_defaults(void){
  ...
  vu_vals_vec = my_calloc(sizeof(float),num_channels*block_size); //OLIVIER
  //frame_buffer = my_calloc(sizeof(float),2*192000/25); //OLIVIER
  frame_buffer = my_calloc(sizeof(float),320*625); //OLIVIER
...
static void move_cursor_to_top(void){
  printf("%c[%dA",0x1b, show_bufferusage ? 1 : 0);
...
static int open_soundfile(void){
  ...
  sf_info.samplerate = custom_samplerate; //OLIVIER
  ...
  case 32: subformat = SF_FORMAT_PCM_32;
    break;
  default:
    if((sf_info.format & SF_FORMAT_SUBMASK)) {
...
static void close_soundfile(void){
  ...
  if (overruns > 0) {
    print_message("jack_capture_video failed with a total of %d overruns.\n", total_overruns);
    print_message("   try a bigger buffer than -B %f\n",min_buffer_time);
  }
  if (disk_errors > 0)
    print_message("jack_capture_video failed with a total of %d disk errors.\n",disk_errors);
  if (total_xruns > 0)
    print_message("jack_capture_video encountered %d jack x-runs.\n", total_xruns);
...
static int disk_write_overruns(int num_overruns){
  if(verbose==true)
    print_message(
                  "jack_capture_video failed writing %d frame%s. Some parts of the recording will contain silence.\n"
...
static void disk_thread_control_priority(void){
  ...
  if(   disk_thread_has_high_priority==false
...
static enum vringbuffer_receiver_callback_return_t disk_callback(vringbuffer_t *vrb,bool first_time,void *element){
  ...
  //disk_write(buffer->data,buffer->pos); //OLIVIER
...
/*static void process_fill_buffer(sample_t *in[],buffer_t *buffer,int i,int end){
  ...
  for(;i<end;i++){
    for(ch=0;ch<num_channels;ch++){
      sample_t val=in[ch][i]; //take the value from "in" (OLIVIER)
      vu_vals_vec[pos] = (float)val; //OLIVIER
      ++frame_buffer_pos;
      if (frame_buffer_pos >= 2*192000/25)
        frame_buffer_pos = 0;
      frame_buffer[frame_buffer_pos] = (float)val;
      // if (frame_buffer_pos >= 192000/25 )
      //   frame_buffer[frame_buffer_pos-192000/25] = -0.5f;
      // else
      //   frame_buffer[frame_buffer_pos+192000/25] = -0.5f;

      data[pos++]=val;        //put it in the buffer...
      val=fabsf(val);
    }
  }
...
static void process_fill_buffer(sample_t *in[],buffer_t *buffer,int i,int end){
  ...
  for(;i<end;i++){
    for(ch=0;ch<num_channels;ch++){
      sample_t val=in[ch][i]; //take the value from "in" (OLIVIER)
      if (val < min_buffer) min_buffer = val;
      if (val > max_buffer) max_buffer = val;

      for (int stretch = 0; stretch < 26; ++stretch) {
        if (stretch+frame_buffer_pos > 200000) {
          frame_buffer_pos -= 200000;
          vu_vals_new=1;
        }
        frame_buffer[stretch+frame_buffer_pos] = (float)val;
      }
      frame_buffer_pos += 26;

      ++pad_buffer_pos;
      if (pad_buffer_pos == 24) { //one more for padding
        pad_buffer_pos = 0;
        ++frame_buffer_pos;
        if (frame_buffer_pos > 200000) {
          frame_buffer_pos -= 200000;
          vu_vals_new=1;
        }
        frame_buffer[frame_buffer_pos] = val;
      }
      pos++;
    }
  }
...
static int process_main(jack_nframes_t nframes, void *arg){ //OLIVIER: renamed for less confusion
  (void)arg;
...
static void jack_shutdown(void *arg){
  (void)arg;
  fprintf(stderr,"jack_capture_video: JACK shutdown.\n");
...
static const char *advanced_help =
  "jack_capture_video  [--bitdepth n] [--channels n] [--port port] [filename]\n"
  "              [ -b        n] [ -c        n] [ -p    port]\n"
  "\n"
  ...
  "           So that \"jack_capture_video ouput.wav\" will produce an wav file, unless a format argument is provided.\n"
  "\n"
  "\n"
  "Additional arguments:\n"
  "[--recording-time n] or [-d n]    -> Recording is stopped after \"n\" seconds.\n"
  "                                    This options also starts jack_capture_video in --no-stdin mode.\n"
  ...
  "                                     Warning: libraries used by jack_capture_video may still print messages.\n"
  ...
  "[--jack-name]/[-jn]               -> Set name of this jack_capture_video instance in the jack patchbay.\n"
  "[--manual-connections]/[-mc]      -> jack_capture_video will not connect any ports for you. \n"
  "[--bufsize s] or [-B s]           -> Initial/minimum buffer size in seconds. Default is 4 seconds.\n"
  "[--maxbufsize] or [-MB]           -> Maximum buffer size in seconds jack_capture_video will allocate.\n"
  ...
  " $jack_capture_video -f wav\n"
  "\n"
  "To record a named stereo file of what you hear in the wav format:\n"
  " $jack_capture_video output.wav\n"
  "\n"
  "To record a stereo file of what you hear in the wav format:\n"
  "  $jack_capture_video --port system:playback_1 --port system:playback_2\n"
  "****************************************************************************\n"
  "**** NOTE! The above example does _exactly_ the same as the default!!!  ****\n"
  "****************************************************************************\n"
  "\n"
  "Same result as above, but using a different syntax:\n"
  "  $jack_capture_video --channels 2 --port system:playback*\n"
  "\n"
  "To record the output from jamin:\n"
  "  $jack_capture_video --port jamin:out* sound_from_jamin.wav\n"
  "\n"
  "To record all sound coming in to jamin:\n"
  "  $jack_capture_video --port jamin:in* sound_to_jamin.wav\n"
  "\n"
  "To record all sound coming in and out of jamin:\n"
  "  $jack_capture_video --port jamin* sound_to_and_from_jamin.wav\n"
  "\n"
  "To record a stereo file from the soundcard:\n"
  "  $jack_capture_video -c 2 -p system:capture*\n"
  "\n";
#endif
...
void init_arguments(int argc, char *argv[]){
  ...
  OPTARGS_BEGIN("\n"
                ...
                "\n"
                "     jack_capture_video --advanced-options (or --help2)\n"
                "\n"
                )
    {
      ...
      OPTARG("--samplerate","-sr") custom_samplerate = OPTARG_GETFLOAT(); //OLIVIER
      ...
      OPTARG("--absolutely-silent","-as") absolutely_silent=true; silent=true; show_bufferusage=false;
      ...
      OPTARG("--disable-console","-dc") show_bufferusage=false;
      OPTARG("--no-stdin","-ns") no_stdin=true;
      OPTARG("--daemon","") no_stdin=true; absolutely_silent=true; silent=true; show_bufferusage=false;
...
char **read_config(int *argc,int max_size){
  ...
  *argc = 0;
  ...
  FILE *file = fopen(string_concat(getenv("HOME"), "/.jack_capture_video/config"),"r");
  if(file==NULL)
    return argv;
...
void init_various(void){
  ...
  // Init jack 2
  {
    jack_set_process_callback(client, process_main, NULL);
